...
async function processToolRequest(
  ...
  // Storage scope for this request's client-uploaded objects. Starts
  // deny-all so a failure before authentication can't delete anything;
  // assigned once `userId` is known. Declared outside the `try` so the
  // `finally` cleanup can use it.
  let uploadScope: StorageScope = { bucket: PDF_UPLOADS_BUCKET, prefixes: [] }
  ...
  try {
    ...
    // Objects this caller may read/delete in Storage. Source files are
    // uploaded under `uploads/<owner>/` (see /api/upload), so anything
    // outside that prefix — another tenant's files, server-owned objects
    // (`jobs/`, `results/`), or the `scan-sessions` bucket — must be
    // rejected even though its public URL is technically fetchable.
    uploadScope = {
      bucket: PDF_UPLOADS_BUCKET,
      prefixes: [ownerUploadPrefix(userId)],
    }
    ...
    // Parse the options payload defensively — a malformed body must yield
    // a 400, not an unhandled `SyntaxError` bubbling up as a 500. A JSON
    // `null`/array is also rejected: the renderers below assume an object.
    ...
    if (typeof blobUrlsRaw === "string" && blobUrlsRaw.length > 0) {
      ...
      for (const entry of parsed) {
        ...
        // The URL must point at an object this caller uploaded. Without
        // this check, a caller could pass any public Storage URL — a
        // victim's result, another tenant's source, a scan-session capture
        // — and have the server read it (and later delete it).
        if (!parseScopedStorageUrl(url, uploadScope)) {
          return NextResponse.json(
            { error: `"${filename}" is not a valid upload for this account` },
            { status: 400 }
          )
        }
    ...
    // Same ownership check as the file blobs above.
    if (watermarkImageUrl && !parseScopedStorageUrl(watermarkImageUrl, uploadScope)) {
      return NextResponse.json(
        { error: "Invalid watermark image reference" },
        { status: 400 }
      )
    }

    const userPlan = userId ? await getUserPlan(userId) : null
    // Per-file cap for this caller's plan (guests get the free limits).
    // Used both to bound each download and for the post-hydration re-check.
    const planLimits = getLimitsForPlan(userPlan ?? "free")
    ...
    // Server-side file-count cap. The client enforces `maxFiles*` for UX,
    // but that is trivially bypassed — an unbounded file list is a
    // memory/CPU DoS and a credit-burn vector for the paid engines.
    {
      const toolConfig = getToolBySlug(tool)
      if (toolConfig) {
        const isPremium = userPlan === "premium"
        const maxFiles = isPremium
          ? toolConfig.maxFilesPremium ?? toolConfig.maxFiles
          : toolConfig.maxFilesFree ?? toolConfig.maxFiles
        if (totalFileCount > maxFiles) {
          return NextResponse.json(
            {
              error: `This tool accepts at most ${maxFiles} file${maxFiles === 1 ? "" : "s"}${isPremium ? "" : " on your plan"}.`,
    ...
    if (blobEntries.length > 0) {
      for (const entry of blobEntries) {
        ...
        try {
          const buffer = await downloadFromStorage(entry.url, uploadScope, {
          ...
          if (err instanceof StorageLimitExceededError) {
            return NextResponse.json(
              {
                error: `"${entry.filename}" exceeds the ${planLimits.maxFileSizeMB} MB limit for your plan.`,
    ...
    if (watermarkImageUrl && !watermarkImage) {
      ...
      try {
        const buffer = await downloadFromStorage(watermarkImageUrl, uploadScope, {
          maxBytes: maxFileBytes,
        })
        ...
        if (err instanceof StorageLimitExceededError) {
          return NextResponse.json(
            {
              error: `The watermark image exceeds the ${planLimits.maxFileSizeMB} MB limit for your plan.`,
              upgradeRequired: !userId,
            },
            { status: 402 }
          )
    ...
    // The `size` values used for the pre-flight gates above come from the
    // client and are therefore untrusted. Now that the real bytes are in
    // memory, re-check them so a caller can't under-report a file's size to
    // slip past the plan cap (or the guest cap).
    {
      const allInputs = watermarkImage ? [...files, watermarkImage] : files
      const oversized = allInputs.find((f) => f.buffer.byteLength > maxFileBytes)
      if (oversized) {
        return NextResponse.json(
          {
            error: `"${oversized.filename}" is ${(oversized.buffer.byteLength / (1024 * 1024)).toFixed(1)} MB, which exceeds the ${planLimits.maxFileSizeMB} MB limit for your plan.`,
            upgradeRequired: !userId,
          },
          { status: 402 }
        )
    ...
    try {
      ...
      let cleanOptions: Record<string, unknown> = { ...options }
      delete cleanOptions._toolSlug
      ...
      // Apply tool-specific parameter mapping. Spread the mapped result
      // back into a plain record so it stays assignable to the loosely
      // typed option bag used by `runTool`.
      ...
      // Log the full upstream error server-side; only a generic message is
      // returned to the client so we don't leak iLoveAPI internals.
      ...
      // Keep the upstream detail in the usage record (server-side) for
      // diagnostics, but never echo it back to the caller.
      let internalMessage = "Processing failed. Please try again."
      if (axiosErr?.response?.data) {
        ...
        if (typeof data === "object" && data !== null) {
          ...
          if (typeof errorObj.message === "string" && errorObj.message.length > 0) {
            ...
            internalMessage = errorObj.error
          } else if (typeof errorObj.error === "object" && errorObj.error !== null) {
            const nestedError = errorObj.error as Record<string, unknown>
            internalMessage = (nestedError.message as string) || (nestedError.error as string) || internalMessage
          }
    ...
    // can't block the others, and refuses anything outside this caller's
    // own namespace.
    await Promise.allSettled(
      downloadedBlobUrls.map((url) => deleteFromStorage(url, uploadScope))
    )
  }
}
